...
for has_lesion_id_weight in $(seq 0.1 0.2 1.1); do
  for p_coarse_dropout in 0.1 0.5; do
    for coarse_dropout_max_holes in 1 16; do
      for coarse_dropout_max_size in 16 32; do
        for downsampling_by_patient in true false; do
          # パラメータを10倍して整数に変換
